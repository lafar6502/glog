            ...
            var ndb = new sqli.Database(nf, function() {
                if (!this.readOnly) {
                    ...
                    var qry = [
                        ...
                        "CREATE TRIGGER IF NOT EXISTS tg_logentry AFTER INSERT ON logentry BEGIN INSERT INTO logentry_fts(docid, message) VALUES(new.rowid, new.message || COALESCE(' ~C' || new.correlation, '') || COALESCE(' ~U' || new.uid, '') || COALESCE(' ~P' || new.pid, '') || COALESCE(' ~T' || new.threadid, '') || COALESCE(' ~L' || new.logname, '') ); END;"
                    ];
    ...
    var writeEvents = function(events, callback) {
        ...
        writeDb(function(tdb) {
            ...
            _.forEach(events, function(evt) {
                ...
                qdata.push("(" + evt.ts + ", " + getLogLevelId(evt.level) + ", " + ns(evt.correlation) + ", " + ns(evt.source) 
