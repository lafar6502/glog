...
var rLogLevels = ['NONE', 'TRACE', 'DEBUG', 'INFO', 'WARN', 'ERROR', 'FATAL'];
var logLevels = {};
...
function LogRepository(cfg) {
        ...
        if (db == null || currentFileName != nf) { //initialize new db
            ...
            var ndb = new sqli.Database(nf, function() {
                if (!this.readOnly) {
                    ...
                    var qry = "CREATE TABLE IF NOT EXISTS logentry(entryid INTEGER NOT NULL PRIMARY KEY, ts INTEGER NOT NULL, level INTEGER NOT NULL, correlation VARCHAR(50), source VARCHAR(50) NOT NULL, send_addr VARCHAR(50) NOT NULL, logname VARCHAR(80) NOT NULL, pid INTEGER NOT NULL, threadid INTEGER NOT NULL,  message TEXT NOT NULL)";
                    ndb.run(qry, function(err) {
    ...
    var writeEvents = function(events, callback) {
        ...
        writeDb(function(tdb) {
            var qdata = [];
            var ssql = "INSERT INTO logentry(ts, level, correlation, source, logname, message, pid, threadid, send_addr) values \n";
            ...
            var doq = function(dt) {
                ...
                console.log('inserting ', dt.length);
            ...
            _.forEach(events, function(evt) {
                ...
                qdata.push("(" + evt.ts + ", " + getLogLevelId(evt.level) + ", " + ns(evt.correlation) + ", " + ns(evt.source) + ", " + ns(evt.logname) + ", " + ns(evt.message) + ", " + (isNaN(evt.pid) ? -1 : evt.pid) + ", " + (isNaN(evt.threadid) ? -1 : evt.threadid) + ", " + ns(evt.send_addr) + ")");
                if (qdata.length >= 500) {
                    doq(qdata);
                    qdata = [];
                };
	...
	this.enqueueEvent = function(evt) {
        ...
        if (!_.has(evt, 'ts') || isNaN(evt.ts)) throw new Error('event.ts must be a number (num of ms since 1970)');
...
function LogSearcher(cfg) {
    ...
    this.search = function(query, start, limit, sort, dir, opts, callback) {
        ...
        var sql = "select entryid, ts, level, correlation, source, logname, message, pid, threadid, send_addr from logentry ";
