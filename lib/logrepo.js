...
function LogRepository(cfg) {
        ...
        if (db == null || currentFileName != nf) { //initialize new db
            ...
            var ndb = new sqli.Database(nf, function() {
                if (!this.readOnly) {
                    ...
                    var qry = "CREATE TABLE IF NOT EXISTS logentry(entryid INTEGER NOT NULL PRIMARY KEY, ts INTEGER NOT NULL, level INTEGER NOT NULL, correlation VARCHAR(50), source VARCHAR(50) NOT NULL, logname VARCHAR(50) NOT NULL, pid INTEGER NOT NULL, threadid INTEGER NOT NULL,  message TEXT NOT NULL)";
    ...
    var writeEvents = function(events, callback) {
        ...
        writeDb(function(tdb) {
            var qdata = [];
            var ssql = "INSERT INTO logentry(ts, level, correlation, source, logname, message, pid, threadid) values \n";
            ...
            _.forEach(events, function(evt) {
                ...
                qdata.push("(" + evt.ts + ", " + getLogLevelId(evt.level) + ", " + ns(evt.correlation) + ", " + ns(evt.source) + ", " + ns(evt.logname) + ", " + ns(evt.message) + ", -1, -1)");
                if (qdata.length >= 500) {
                    doq(qdata);
                    qdata = [];
                };
...
function LogSearcher(cfg) {
    ...
    this.search = function(query, start, limit, sort, dir, opts, callback) {
        ...
        var sql = "select entryid, ts, level, correlation, source, logname, message from logentry ";
