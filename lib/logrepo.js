...
function LogRepository(cfg) {
    ...
    var writeEvents = function(events, callback) {
        ...
        writeDb(function(tdb) {
            var qdata = [];
            var ssql = "INSERT INTO logentry(ts, level, correlation, source, logname, message) values \n";
            ...
            _.forEach(events, function(evt) {
                ...
                qdata.push("(" + evt.ts + ", " + getLogLevelId(evt.level) + ", " + ns(evt.correlation) + ", " + ns(evt.source) + ", " + ns(evt.logname) + ", " + ns(evt.message) + ")");
                if (qdata.length >= 500) {
                    doq(qdata);
                    qdata = [];
                };
...
function LogSearcher(cfg) {
    var db = new sqli.Database(cfg.fileName, sqlite3.OPEN_READONLY, function(err) {
    });
